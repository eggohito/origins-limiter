...
#
#   >   Check if the player has canceled origin selection in the `origins-classes:class` origin layer
#
...
execute if data entity @s cardinal_components."origins:origin".OriginLayers[{Origin: "origins-limiter:cancel", Layer: "origins-limiter:restrict/origins-classes/class"}] run tag @s add origins-limiter.canceled.tmp
...
execute if entity @s[tag = origins-limiter.canceled.tmp] run tellraw @s {"text": "Origin selection on 'origins-classes:class' origin layer has been canceled.", "color": "red"}

execute if entity @s[tag = origins-limiter.canceled.tmp] run origin set @s origins-classes.class origins-limiter:empty
